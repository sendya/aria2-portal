...
export function setI18nLanguage(lang: string = defaultLang): void {
  i18n.global.locale.value = lang;
  localStorage.setItem('app-i18n', lang);
...
export async function loadLanguageAsync(lang: string = defaultLang) {
  ...
  if (currentI18n.locale.value !== lang) {
    if (!currentI18n.availableLocales.includes(lang)) {
